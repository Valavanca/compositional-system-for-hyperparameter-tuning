...
import os
import logging
logging.basicConfig(filename='./scale_tutor_dtlz.log', level=logging.INFO)
...
def tuning_loop(pro, udp,
    ...
    tutor = PredictTutor(pro.get_bounds(),
                         ...
                         solver=solver,
                         solver_gen=50*pro.get_nobj(),
                         solver_pop=50*pro.get_nobj(),
    ...
    while i < n_iter:
        ...
        pred['portfolio'] = [type(m).__name__ for m in surr_portfolio]
        pred['n_pred'] = n_pred
    ...
    rel_path = '/benchmark_results/{}_{}{}_{}_scale_tutor_loop.{}'.format(
        pro.get_name(), pro.get_nx(), pro.get_nobj(), n_pred, loop_prefix)
    path = os.path.dirname(os.path.abspath(__file__))

    # Write results
    print(" Write scale loop csv file. Path:{}".format(path + rel_path + '.csv'))
    loop.to_csv(path + rel_path + '.csv', mode='a+', index=False)

    print(" Write scale loop pkl file. Path:{}".format(path + rel_path + '.pkl'))
...
def experiment(problem_name: str,
    ...
    pred_count = pred_count*obj
    eval_budget = eval_budget*obj
    ...
    result = {
        ...
        'port_names': [type(m).__name__ for m in surr_port],
    ...
    try:
        ...
        result["evol_loop_time"] = t_end - evolve_start
...
if __name__ == "__main__":
    logging.info(
        "----\n Start scale benchamrk on all multy-objective problems\n ---- ")

    print("Start scale DTLZ")
    ...
    # 5
    mlp_reg_w50d5 = MLPRegressor(hidden_layer_sizes=(
        50, 50, 50, 50, 50), activation='relu', solver='lbfgs')
    mlp_uni_50x5 = ModelsUnion(models=[mlp_reg_w50d5], split_y=True)


    # 6
    mlp_reg_w100d5 = MLPRegressor(hidden_layer_sizes=(
        100, 100, 100, 100, 100), activation='relu', solver='lbfgs')
    mlp_uni_100x5 = ModelsUnion(models=[mlp_reg_w100d5], split_y=True)
    ...
    test_set = [
        {
            ...
            'eval_budget': [500],
            'pred_count': [50],
            'surr_port': [[gp_mauna], [mlp_reg_w50d5], [mlp_reg_w100d5], [grad_uni, svr_uni, mlp_uni_50x5]],
            'solver': ['moea_control'],
            'train_test_sp': [0.25],
            'cv_threshold': ['(test_r2 > 0.2)'],
            'test_threshold': ['(ndf_surr_score > 0)'],
            'solution_comb': ['stack'],
            'start_set': [0],
            'seed': [SEED]
        }
