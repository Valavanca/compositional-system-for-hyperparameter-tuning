...
optimization objectives. Therefore, the performance comparison of
...
Those approaches include: scalarization and . We describe them in next
the sections.
...
some way to become a new population in the next iteration. However,
...
evaluation budget makes s infeasible for costly and multi-objective
problems.
...
surrogate models. For an initial sampling plan, or a plan, the
techniques of , Sobol sampling or random sampling can be used.
...
It could be an interpreted as a in model-based optimization. If we
